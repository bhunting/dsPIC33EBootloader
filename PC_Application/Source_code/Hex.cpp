...
void CHexManager::VerifyFlash(unsigned int* StartAdress, unsigned int* ProgLen, unsigned short* crc)
{
	...
	// Virtual Flash Erase (Set bytes in the pattern  0x00FFFFFF)
	for(int i = 0; i < sizeof(VirtualFlash); i++)
	{
		if((i+1) % 4)
		{
			VirtualFlash[i] = 0xFF;
    ...
    while((HexRecLen = GetNextHexRecord(&HexRec[0], 255)) != 0)
	{
		...
		switch(HexRecordSt.RecType)
		{
			...
			case DATA_RECORD:  //Record Type 00, data record.
				...
				ProgAddress = HexRecordSt.Address;
				...
				if(ProgAddress < BOOT_SECTOR_BEGIN) // Make sure we are not writing boot sector.
				{
					...
					VirtualFlashAdrs = ProgAddress; // Program address to local virtual flash address
	...
	*StartAdress = HexRecordSt.MinAddress/2;
	VirtualFlashAdrs = HexRecordSt.MinAddress;
